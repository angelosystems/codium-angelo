...
export APP_NAME="Codium Angelo"
export ASSETS_REPOSITORY="angelosystems/codium-angelo"
export BINARY_NAME="codium-angelo"
export CI_BUILD="no"
export GH_REPO_PATH="angelosystems/codium-angelo"
export ORG_NAME="Angelo Systems"
...
while getopts ":ilops" opt; do
  case "$opt" in
    i)
      export ASSETS_REPOSITORY="angelosystems/codium-angelo-insiders"
      export BINARY_NAME="codium-angelo-insiders"
...
if [[ "${SKIP_BUILD}" == "no" ]]; then
  ...
  if [[ -f "./include_${OS_NAME}.gypi" ]]; then
    ...
    if [[ -f "${HOME}/.gyp/include.gypi" ]]; then
      mv ~/.gyp/include.gypi ~/.gyp/include.gypi.pre-angelo-x
    else
      echo "{}" > ~/.gyp/include.gypi.pre-angelo-x
    fi
  ...
  if [[ -f "./include_${OS_NAME}.gypi" ]]; then
    mv ~/.gyp/include.gypi.pre-angelo-x ~/.gyp/include.gypi
  fi
